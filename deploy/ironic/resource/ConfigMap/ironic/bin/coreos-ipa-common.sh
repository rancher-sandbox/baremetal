...
function coreos_kernel_params {
    echo -n "coreos.live.rootfs_url=${IRONIC_BOOT_BASE_URL}/images/ironic-python-agent.rootfs"
    if [ -f "$IGNITION_FILE" ]; then
        echo -n " ignition.config.url=${IRONIC_BOOT_BASE_URL}/ironic-python-agent.ign"
    fi
